...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while (true)
	{
		...
		if (GetAsyncKeyState(0x32) != 0) // user pressed 2
		{
			...
			cout << "TYRE COMPOUND : " << pf->tyreCompound << endl;
		...
		if (GetAsyncKeyState(0x33) != 0) // user pressed 3
		{
			...
			cout << "SM VERSION " << pf->smVersion << endl;
			cout << "AC VERSION " << pf->acVersion << endl;

			printData("number of sessions ", pf->numberOfSessions);
			printData("numCars", pf->numCars);
			cout << "Car model " << pf->carModel << endl;
			cout << "Car track " << pf->track << endl;
			cout << "Player Name " << pf->playerName << endl;
			// cout << "Player Name " << pf->playerNick << endl;
			// cout << "Player Name " << pf->playerSurname << endl;
